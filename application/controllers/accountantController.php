...
class AccountantController extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $config = array();
        $config["base_url"] = base_url()."accountant";
        ...
        $this->data["paymentsForThisMonth"]=$this->AccountantModel->loadThisMonthPayment($config["per_page"], $page);  //loading current month payments from database
    ...
    {
        $this->load->model('AccountantModel');
        $this->AccountantModel->addPaymentRecord($this->input->post()); //updating selected record from database
